 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2021-10-10 15:58:35
 */
...
bool perception_filter::Whether_OK( perception per) {
        ...
        for(auto iter = per_list.end(); iter != per_list.begin(); iter-- ){
            if(iter->person_name_ == per.person_name_  &&  iter->IDtype_ == per.IDtype_ &&  iter->person_name_ == per.person_name_ &&  iter->intention_ == per.intention_   ){
